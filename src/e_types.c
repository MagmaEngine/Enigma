...
void e_xyz_to_rgb3(float *output, float x, float y, float z)
{
	float tmp[3];
	tmp[0] = x / 100.0f;
	tmp[1] = y / 100.0f;
...
#define e_WHITEPOINT_X	0.950456
#define e_WHITEPOINT_Y	1.0
#define e_WHITEPOINT_Z	1.088754
...
void e_xyz_to_rgb(float *rgb, float x, float y, float z)
{
	float min;
...
void e_xyz_to_lab(float *lab, float x, float y, float z)
{
	x /= e_WHITEPOINT_X;
	y /= e_WHITEPOINT_Y;
	z /= e_WHITEPOINT_Z;
...
void e_lab_to_xyz(float *xyz, float l, float a, float b)
{
	l = (l + 16.0f) / 116.0f;
	a = l + a / 500.0f;
	b = l - b / 200.0f;
...
void e_rgb_to_lab(float *lab, float r, float g, float b)
{
	float tmp[3];
	e_rgb_to_xyz(tmp, r, g, b);
	e_xyz_to_lab(lab, tmp[0], tmp[1], tmp[2]);
}

void e_lab_to_rgb(float *rgb, float l, float a, float b)
{
	float tmp[3];
	e_lab_to_xyz(tmp, l, a, b);
	e_xyz_to_rgb(rgb, tmp[0], tmp[1], tmp[2]);
}



float e_splinef(float f, float v0, float v1, float v2, float v3)
{
	float inv;
	inv = 1.0f - f;
...
double e_splined(double f, double v0, double v1, double v2, double v3)
{
	double inv;
	inv = 1.0f - f;
	return ((v3 * f + v2 * inv) * f + (v2 * f + v1 * inv) * inv) * f + ((v2 * f + v1 * inv) * f + (v1 * f + v0 * inv) * inv) * inv;
...
void e_spline2df(float *out, float f, float *v0, float *v1, float *v2, float *v3)
{
	float inv;
	inv = 1.0f - f;
...
void e_spline2dd(double *out, double f, double *v0, double *v1, double *v2, double *v3)
{
	double inv;
	inv = 1.0f - f;
	out[0] = ((v3[0] * f + v2[0] * inv) * f + (v2[0] * f + v1[0] * inv) * inv) * f + ((v2[0] * f + v1[0] * inv) * f + (v1[0] * f + v0[0] * inv) * inv) * inv;
...
void e_spline3df(float *out, float f, float *v0, float *v1, float *v2, float *v3)
{
	float inv;
	inv = 1.0f - f;
	out[0] = ((v3[0] * f + v2[0] * inv) * f + (v2[0] * f + v1[0] * inv) * inv) * f + ((v2[0] * f + v1[0] * inv) * f + (v1[0] * f + v0[0] * inv) * inv) * inv;
...
void e_spline3dd(double *out, double f, double *v0, double *v1, double *v2, double *v3)
{
...
void e_spline4df(float *out, float f, float *v0, float *v1, float *v2, float *v3)
{
	float inv;
	inv = 1.0f - f;
	out[0] = ((v3[0] * f + v2[0] * inv) * f + (v2[0] * f + v1[0] * inv) * inv) * f + ((v2[0] * f + v1[0] * inv) * f + (v1[0] * f + v0[0] * inv) * inv) * inv;
...
void e_spline4dd(double *out, double f, double *v0, double *v1, double *v2, double *v3)
{
	double inv;
	inv = 1.0 - f;
	out[0] = ((v3[0] * f + v2[0] * inv) * f + (v2[0] * f + v1[0] * inv) * inv) * f + ((v2[0] * f + v1[0] * inv) * f + (v1[0] * f + v0[0] * inv) * inv) * inv;
...
float e_wigglef(float f, float size)
{
	float v0, v1, v2, v3;
	uint seed;
	seed = (float)f;
	...
	seed *= 2;
	size *= 2.0;
	v0 = (e_randf(seed++) - 0.5) * size;
	v1 = (e_randf(seed++) - 0.5) * size;
	v3 = (e_randf(seed++) - 0.5) * size;
	v2 = v3 * 2.0 - (e_randf(seed++) - 0.5) * size;
	return e_spline(f, v0, v1, v2, v3);
}

double e_wiggled(double f, double size)
{
	double v0, v1, v2, v3;
	uint seed;
	seed = (double)f;
	...
	seed *= 2;
	size *= 2.0;
	v0 = (e_randd(seed++) - 0.5) * size;
	v1 = (e_randd(seed++) - 0.5) * size;
	v3 = (e_randd(seed++) - 0.5) * size;
	v2 = v3 * 2.0 - (e_randd(seed++) - 0.5) * size;
	return e_spline(f, v0, v1, v2, v3);
}

void e_wiggle2df(float *out, float f, float size)
{
	...
	seed *= 4;
	size *= 2.0;
	v0[0] = (e_randf(seed++) - 0.5) * size;
	v0[1] = (e_randf(seed++) - 0.5) * size;
	v1[0] = (e_randf(seed++) - 0.5) * size;
	v1[1] = (e_randf(seed++) - 0.5) * size;
	v3[0] = (e_randf(seed++) - 0.5) * size;
	v3[1] = (e_randf(seed++) - 0.5) * size;
	v2[0] = v3[0] * 2.0 - (e_randf(seed++) - 0.5) * size;
	v2[1] = v3[1] * 2.0 - (e_randf(seed++) - 0.5) * size;
	e_spline2df(out, f, v0, v1, v2, v3);
}

void e_wiggle2dd(double *out, double f, double size)
{
	...
	seed *= 4;
	size *= 2.0;
	v0[0] = (e_randd(seed++) - 0.5) * size;
	v0[1] = (e_randd(seed++) - 0.5) * size;
	v1[0] = (e_randd(seed++) - 0.5) * size;
	v1[1] = (e_randd(seed++) - 0.5) * size;
	v3[0] = (e_randd(seed++) - 0.5) * size;
	v3[1] = (e_randd(seed++) - 0.5) * size;
	v2[0] = v3[0] * 2.0 - (e_randd(seed++) - 0.5) * size;
	v2[1] = v3[1] * 2.0 - (e_randd(seed++) - 0.5) * size;
	e_spline2dd(out, f, v0, v1, v2, v3);
}

void e_wiggle3df(float *out, float f, float size)
{
	...
	v0[0] = (e_randf(seed++) - 0.5) * size;
	v0[1] = (e_randf(seed++) - 0.5) * size;
	v0[2] = (e_randf(seed++) - 0.5) * size;
	v1[0] = (e_randf(seed++) - 0.5) * size;
	v1[1] = (e_randf(seed++) - 0.5) * size;
	v1[2] = (e_randf(seed++) - 0.5) * size;
	v3[0] = (e_randf(seed++) - 0.5) * size;
	v3[1] = (e_randf(seed++) - 0.5) * size;
	v3[2] = (e_randf(seed++) - 0.5) * size;
	v2[0] = v3[0] * 2.0 - (e_randf(seed++) - 0.5) * size;
	v2[1] = v3[1] * 2.0 - (e_randf(seed++) - 0.5) * size;
	v2[2] = v3[2] * 2.0 - (e_randf(seed++) - 0.5) * size;
	e_spline3df(out, f, v0, v1, v2, v3);
}

void e_wiggle3dd(double *out, double f, double size)
{
	...
	v0[0] = (e_randd(seed++) - 0.5) * size;
	v0[1] = (e_randd(seed++) - 0.5) * size;
	v0[2] = (e_randd(seed++) - 0.5) * size;
	v1[0] = (e_randd(seed++) - 0.5) * size;
	v1[1] = (e_randd(seed++) - 0.5) * size;
	v1[2] = (e_randd(seed++) - 0.5) * size;
	v3[0] = (e_randd(seed++) - 0.5) * size;
	v3[1] = (e_randd(seed++) - 0.5) * size;
	v3[2] = (e_randd(seed++) - 0.5) * size;
	v2[0] = v3[0] * 2.0 - (e_randd(seed++) - 0.5) * size;
	v2[1] = v3[1] * 2.0 - (e_randd(seed++) - 0.5) * size;
	v2[2] = v3[2] * 2.0 - (e_randd(seed++) - 0.5) * size;
	e_spline3dd(out, f, v0, v1, v2, v3);
}
...
bool e_raycast_tri_cullf(float orig[3], float dir[3], float vert0[3], float vert1[3], float vert2[3], float *t, float *u, float *v)
{
	float edge1[3], edge2[3], tvec[3], pvec[3], qvec[3];
	float det, inv_det;
...
bool e_raycast_trid(double orig[3], double dir[3], double vert0[3], double vert1[3], double vert2[3], double *t, double *u, double *v)
{
...
bool e_raycast_tri_culld(double orig[3], double dir[3], double vert0[3], double vert1[3], double vert2[3], double *t, double *u, double *v)
{
	double edge1[3], edge2[3], tvec[3], pvec[3], qvec[3];
	double det, inv_det;
